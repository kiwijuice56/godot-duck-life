...
@export var remove_debris = false
@export var collision_layers = 1
@export var collision_masks = 1
@export var collision_one_way = false
@export var explosion_delay = false
@export var fake_explosions_group = "fake_explosion_particles"
...
func _ready() -> void:
	...
	object.blocks_container.name = str(self.name) + "_blocks_container"
	...
	if object.sprite_name == "" and object.collision_name == "":
	...
	get_node(str(object.sprite_name)).vframes = object.blocks_per_side
	get_node(str(object.sprite_name)).hframes = object.blocks_per_side
	object.vframes = get_node(str(object.sprite_name)).vframes
	object.hframes = get_node(str(object.sprite_name)).hframes
	...
	if get_node(str(object.sprite_name)).region_enabled:
		object.width = float(get_node(str(object.sprite_name)).region_rect.size.x)
		object.height = float(get_node(str(object.sprite_name)).region_rect.size.y)
	else:
		object.width = float(get_node(str(object.sprite_name)).texture.get_width())
		object.height = float(get_node(str(object.sprite_name)).texture.get_height())
	...
	if get_node(str(object.sprite_name)).centered:
	...
	for n in range(object.vframes * object.hframes):
		...
		duplicated_object.name = str(self.name) + "_block_" + str(n)
		...
		object.blocks[n].get_node(str(object.sprite_name)).vframes = object.vframes
		object.blocks[n].get_node(str(object.sprite_name)).hframes = object.hframes
		object.blocks[n].get_node(str(object.sprite_name)).frame = n
		object.blocks[n].get_node(str(object.collision_name)).shape = shape
		object.blocks[n].get_node(str(object.collision_name)).position = object.collision_position

		if object.collision_one_way: object.blocks[n].get_node(str(object.collision_name)).one_way_collision = true
...
func detonate():
	...
	for child in object.parent.get_children():
		if child is CPUParticles2D or child.is_in_group(fake_explosions_group):
			object.particles = child
			object.has_particles = true
	if object.has_particles:
		if object.particles is CPUParticles2D:
	...
	for i in range(object.blocks_container.get_child_count()):
		...
		var child_scale = randf_range(0.25, .8)
		child.get_node(str(object.sprite_name)).scale = Vector2(child_scale, child_scale)
		child.get_node(str(object.collision_name)).scale = Vector2(child_scale, child_scale)
...
func _on_debris_timer_timeout():
	...
	for i in range(object.blocks_container.get_child_count()):
		...
		if not object.remove_debris:
			child.freeze = true
			child.get_node(str(object.collision_name)).disabled = true
